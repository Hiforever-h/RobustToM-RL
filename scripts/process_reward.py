#!/usr/bin/env python3
"""Deterministically score structured ToM process outputs without an LLM judge."""
...
def normalize(value: Any) -> str:
    """Normalize finite-vocabulary labels without performing semantic matching."""
...
def score_process_output(
    output: str | dict[str, Any], target: dict[str, Any]
) -> dict[str, Any]:
    """Return the scalar reward plus per-stage checks and contributions."""
    ...
    order_ok = (
        type(prediction.get("tom_order")) is int
        and prediction["tom_order"] == target["tom_order"]
    )
    chain_ok = normalized_chain(prediction.get("belief_chain")) == normalized_chain(
        target["belief_chain"]
    )
    ...
    if mode == "world_state":
        world_ok = normalize(prediction.get("world_state", "")) == normalize(
            target["world_state"]
        )
        ...
        belief_ok = normalize(prediction.get("nested_belief", "")) == normalize(
            target["nested_belief"]
        )
        answer_ok = normalize(prediction.get("answer", "")) == normalize(target["answer"])
        checks = {
            "parseable_json": True,
...
def process_reward(output: str | dict[str, Any], target: dict[str, Any]) -> float:
    """Trainer-friendly scalar reward callback."""
...
def main() -> None:
    args = parse_args()
    prediction = args.prediction.read_text(encoding="utf-8")
    target = json.loads(args.target.read_text(encoding="utf-8"))
    print(json.dumps(score_process_output(prediction, target), indent=2))
